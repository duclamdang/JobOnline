...
class GroqController extends Controller
{
    ...
    {
        try {
            $messages = $this->truncateMessages($req->input('messages', []), self::MAX_HISTORY_CHARS);
            ...
            if (preg_match('/(#|\bid\s*[:\-]?\s*)(\d{1,10})/iu', $userText, $m)) {
                $intent = ['intent' => 'job_detail', 'id' => (int)$m[2], 'page' => 1];
            } else {
                $intent = $noLlm ? $this->regexIntent($userText)
            ...
            // Nếu không có KEY (hoặc muốn tắt LLM) -> trả luôn context (200)
            if ($noLlm) {
            ...
            // System + CONTEXT
            $system = "Bạn là trợ lý tuyển dụng JobOnline. Trả lời TIẾNG VIỆT, ngắn gọn, chỉ dựa vào CONTEXT.\n"
                . "- Nếu CONTEXT rỗng: nói rõ 'không tìm thấy' và gợi ý lọc khác.\n"
                . "- Không tự bịa công ty/lương/vị trí nếu không có trong CONTEXT.\n"
                . "- Khi nói tới MÃ tin (#id), nhắc đúng #id trong CONTEXT.\n";
            if ($context) $system .= "\n=== CONTEXT (từ CSDL) ===\n{$context}\n=== HẾT CONTEXT ===\n";

            $final = [['role' => 'system', 'content' => $system]];
            foreach ($messages as $m) if (isset($m['role'], $m['content'])) $final[] = $m;

            // Gọi Groq: luôn trả 200 kể cả lỗi
            $resp = Http::withToken(env('GROQ_API_KEY'))
                ->timeout(30)->connectTimeout(10)->retry(2, 200)
            ...
            if ($resp->failed()) {
                \Log::error('Groq error', ['status'=>$resp->status(),'body'=>$resp->body(),'intent'=>$intent]);
                return response()->json([
                    'text'     => $this->fallbackFromContext($context),
                    'metadata' => $meta,
...
Chỉ JSON, không thêm chữ nào khác.";
            $res = Http::withToken(env('GROQ_API_KEY'))
                ->timeout(15)->connectTimeout(8)->retry(1, 200)
                ->post('https://api.groq.com/openai/v1/chat/completions', [
                    'model'    => env('GROQ_MODEL', self::MODEL),
                    'messages' => [
            ...
            if (!is_array($json) || empty($json['intent'])) return null;
    ...
    private function regexIntent(string $text): array
    {
        ...
        if (preg_match('/(#|\bid\s*[:\-]?\s*)(\d{1,10})/iu', $text, $m)) {
            return ['intent' => 'job_detail', 'id' => (int)$m[2], 'page' => 1];
        }

        $searchKw = ['tìm','việc','job','tuyển','lập trình','kế toán','nhân viên','dev','tester','data','marketing','designer','thiết kế'];
        $has = collect($searchKw)->contains(fn($k) => Str::contains($t, $k));

        if ($has) {
            $city = collect(['đà nẵng','da nang','hồ chí minh','ho chi minh','hcm','hà nội','ha noi','bình dương','binh duong','cần thơ','can tho','hải phòng','hai phong'])
                ->first(fn($c) => Str::contains($t,$c));
            $company = null;
            if (preg_match('/(công ty|cty)\s+([a-zA-Z0-9\p{L}\s\.\-]+)/u', $text, $m)) $company = trim($m[2]);
            $kw = null; if (preg_match('/(java|python|php|react|flutter|kế toán|marketing|thiết kế|data|tester)/iu',$text,$m2)) $kw=$m2[1];

            return [
                'intent' => 'search_jobs', 'query'=>$kw, 'city'=>$city, 'company'=>$company,
                'salaryMin'=>null,'salaryMax'=>null,'expMin'=>null,'expMax'=>null,
                'type'=>null,'remote'=>null,'postedWithinDays'=>null,'page'=>1,'id'=>null,
            ];
        }
    ...
    private function resolveContext(array $intent): array
    {
        $debug = ['intent'=>$intent];
        $ctx   = '';
        $meta  = ['intent'=>$intent['intent'] ?? 'chitchat', 'page'=>(int)($intent['page'] ?? 1), 'total'=>0, 'pages'=>1];

        switch ($intent['intent'] ?? 'chitchat') {
            case 'job_detail':
                $id = (int)($intent['id'] ?? 0);
                $job = $this->jobById($id);
                if ($job) $ctx = $this->formatJobDetail($job);
                break;
            ...
            case 'search_jobs':
                ...
                $debug['page']=$page; $debug['total']=$total;
                $meta['page']=$page; $meta['total']=$total; $meta['pages']=(int)ceil(max(1,$total)/self::PAGE_SIZE);
                break;
    ...
    {
        return DB::table('jobs')->where('is_active',1)->where('id',$id)->first();
    }

    /**
     * Tìm trực tiếp trong DB (PostgreSQL)
     * - work_field_id: JSON/JSONB mảng ID -> match ANY (an toàn với NULL/không phải array)
    ...
    private function searchJobsFromNL(array $intent, int $page = 1, int $per = 10): array
    {
        ...
        if (Schema::hasColumn('jobs','work_field_id')) {
            $fieldIds = $this->resolveWorkFieldIdsFromKeyword($intent['query'] ?? null);
            if (!empty($fieldIds)) {
                $arr = implode(',', array_map('intval', $fieldIds)); // "1,7"
                $q->whereRaw("
                    EXISTS (
                      SELECT 1
                ...
                ");
                // Nếu muốn bắt buộc chứa đủ tất cả fieldIds -> dùng:
                // $q->whereRaw('(jobs.work_field_id::jsonb @> \'['.$arr.']\')');
            }
        }
        ...
        // kinh nghiệm nếu có cột số năm (tuỳ DB)
        ...
        $items = $q->orderByDesc('created_at')
            ...
            ->get(['id','title','company_name','location','salary_min','salary_max','deadline','job_type','is_remote']);
    ...
    private function money($v): string { return $v ? number_format((int)$v, 0, ',', '.') : '-'; }
    ...
    private function formatJobList($jobs, int $total, int $page, int $per): string
    {
        ...
        foreach ($jobs as $j) {
            $lines[] = sprintf(
                ...
                $this->money($j->salary_min), $this->money($j->salary_max),
                $j->deadline ? date('d/m/Y', strtotime($j->deadline)) : '-',
                $j->job_type ?? '-', ($j->is_remote ? 'Remote' : 'Onsite')
            );
        }
    ...
    private function norm(string $s): string { return trim(Str::lower(Str::ascii($s))); }

    private function dict(string $table, array $cols = ['id','name'])
    {
        return Cache::remember("dict:$table", now()->addMinutes(10), fn() =>
        DB::table($table)->get($cols)
        );
    }
    ...
    private function resolveProvinceId(?string $city): ?int
    {
        ...
        $rows = $this->dict('provinces'); // ĐỔI tên bảng nếu khác
        foreach ($rows as $r) {
            $n = $this->norm($r->name);
            if (Str::contains($t,$n) || Str::contains($n,$t)) return (int)$r->id;
        }
        foreach ($alias as $canon => $alts) {
            ...
            foreach ($alts as $a) {
                if (Str::contains($t, $this->norm($a))) {
                    $row = $rows->first(fn($r)=>Str::contains($this->norm($r->name), explode(' ', $canonN)[1] ?? $canonN));
                    if ($row) return (int)$row->id;
                }
            }
        }
    ...
    // work_field_id: IDs theo keyword/synonyms (bảng danh mục: work_fields)
    private function resolveWorkFieldIdsFromKeyword(?string $kw): array
    {
        ...
        $rows = $this->dict('work_fields'); // ĐỔI nếu khác
        $hit = [];
        foreach ($rows as $r) {
            $n = $this->norm($r->name);
            if (Str::contains($n,$t) || Str::contains($t,$n)) $hit[$r->id]=true;
            foreach ($syn as $k=>$alts) {
                if (Str::contains($t,$k) && Str::contains($n,$k)) $hit[$r->id]=true;
                foreach ($alts as $a) if (Str::contains($t,$this->norm($a)) && Str::contains($n,$k)) $hit[$r->id]=true;
            }
        }
    ...
    private function extractFirstJsonObject(string $raw): string
    {
        ...
        return ($s !== false && $e !== false && $e >= $s) ? substr($raw, $s, $e-$s+1) : '{}';
    }
}
